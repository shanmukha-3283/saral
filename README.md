...
- Infra (AWS SAM, `template.yaml`): API Gateway (HTTP API), Lambda, S3
  (uploads, 24h lifecycle), DynamoDB (`saral-results`), Amazon Bedrock
  (Claude Sonnet vision + explanation)
...
Amazon Bedrock (Anthropic Claude Sonnet) · AWS CloudFormation (via SAM) ·
Amazon CloudFront (phase 4)
